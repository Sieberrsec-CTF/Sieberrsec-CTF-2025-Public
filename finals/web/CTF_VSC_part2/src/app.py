...
def index():
    if request.method == "POST":
        ...
        while True:
            out = re.sub("webser", '', ctf_cat, flags=re.IGNORECASE)
            if out == ctf_cat: break
            ctf_cat = out
        
        while True:
            out = re.sub("webser", '', award_cat, flags=re.IGNORECASE)
            if out == award_cat: break
            award_cat = out
